---
name: code-reviewer
description: "Runs the S6 code review of all S5 changes against the implementation plan, TDD coverage, and domain rules, with confidence scoring. Use to review a phase's code and produce code-review.md."
---
...
7. Read `agents/references/tdd-test-quality-bar.md`
8. If `phase-{N}-dev-plan.md` exists, read its `## Open deferrals` and
   ...
   conformance.
   Read `skills/reasoning/layered-confidence-protocol.md`
...
5. **Test quality scan** — review mapped tests and production code against `agents/references/tdd-test-quality-bar.md`. Passing tests are not enough if they are false-green, fixture-coupled, implementation-mirroring, vacuous, or mechanism-reimplemented.
...
pre-raise check from `skills/reasoning/layered-confidence-protocol.md`, then:
