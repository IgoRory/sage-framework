...
description: "Builds the S2 implementation plan — maps every TDD scenario to a test file and assertion, lists touched files, and (Checkpoint mode) defines batches. Use after the dev interview to plan the build."
---
...
8. Read `agents/references/tdd-test-quality-bar.md`
...
If the scoped files touch `ProfitabilityAPI.V2` or `ProfitabilityWeb` and `skills/write-tests/SKILL.md` is available in the active product repo, read it before mapping tests. If the skill is unavailable, use the fallback guidance below.
...
Every TDD task must include a `Test quality contract` using `agents/references/tdd-test-quality-bar.md`. The contract is part of the implementation plan, not optional commentary.
