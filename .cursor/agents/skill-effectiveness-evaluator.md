...
4. Read each SKILL.md in `.cursor/skills/` to understand current content
...
When **`prd-interviewer-effectiveness-evaluator`** is present in `.cursor/skills/`, **do not evaluate `prd-interviewer`** here — the PRD JSONL-based evaluator owns that skill.
