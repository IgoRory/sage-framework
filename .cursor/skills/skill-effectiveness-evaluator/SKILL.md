...
| All SKILL.md files | .cursor/skills/**/SKILL.md | Yes |

---

## Coordination with prd-interviewer-effectiveness-evaluator

When **`.cursor/skills/prd-interviewer-effectiveness-evaluator/SKILL.md`** exists in the repository, **`prd-interviewer` is evaluated only by that skill**. Skip the entire **### prd-interviewer** subsection below in Step 2 — do not propose duplicate SKILL updates or Linear issues for **`prd-interviewer`**. PRD telemetry lives in **`prd.telemetryFile`** (see `.sage/workflow-config.json`), not session **`workflow-telemetry.jsonl`**.
...
``````diff
--- a/.cursor/skills/[skill-name]/SKILL.md
+++ b/.cursor/skills/[skill-name]/SKILL.md
